...
- [x] T7 Extend `.github/workflows/ci.yml`: `dotnet tool restore`, then `dotnet csharpier . --check` and `dotnet format VoiceTypingToggle.slnx --verify-no-changes` steps; add `.config/dotnet-tools.json` to the setup-dotnet cache-dependency-path; run the check commands locally to confirm they pass on the formatted repo
